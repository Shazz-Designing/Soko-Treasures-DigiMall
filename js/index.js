...
function updateCartDisplay() {
  ...
  shoppingCart.forEach((item) => {
    ...
    cartItem.textContent = `${item.name} x${item.quantity} - Ksh ${(
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const viewCartModal = new bootstrap.Modal(
  ...
  function handleCheckout() {
    ...
    const cartTotalValue = parseFloat(
      cartTotalText.replace("Ksh ", "").replace(/,/g, "")
    ); // Remove 'Ksh ' and parse to a float
